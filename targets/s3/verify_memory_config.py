#!/usr/bin/env python3
"""Fail configure when IDF silently drops the existing N16R8 memory contract.

Authority: config/sdkconfig.defaults.esp32s3 and docs/memory.md. The module has
8 MiB physical octal PSRAM; ECC reserves 1/16, leaving 0x780000 addressable bytes.
...
CONTRACT_DEFAULTS = ROOT / "config/sdkconfig.defaults.esp32s3"
REQUIRED_TRUE = (
...
# These must not bypass required RAM or change the accepted quad-flash/octal-RAM
...
def configuration_errors(
    ...
    for key in REQUIRED_TRUE:
        for label, values in (("canonical defaults", canonical),
                              ("S3 defaults", target_defaults), ("resolved sdkconfig", config)):
            if values.get(key) != "y":
                errors.append(f"{label}: {key} must be y (got {values.get(key)!r})")
    for key in REQUIRED_FALSE:
        if config.get(key, "n") != "n":
            errors.append(f"resolved sdkconfig: {key} must be disabled")
    ...
    expected_partition = (root / "config/partitions_16m.csv").resolve()
    for label, values, base in (("canonical defaults", canonical, root),
                                ("S3 defaults", target_defaults, project),
        ...
        if not (value.startswith('"') and value.endswith('"')):
            ...
            errors.append(f"{label}: custom partition must resolve to config/partitions_16m.csv")
    return errors
...
def main() -> int:
    ...
    if errors:
        ...
        return 1
    print("S3 resolved configuration: 16-MiB flash, octal PSRAM 80 MHz, boot init + ECC; "
