...
class ProductSiteTests(unittest.TestCase):
    ...
    def test_landing_pages_describe_capabilities_not_project_history(self):
        ...
        landing_pages = {
            "README.md": (
                "Firmware roadmap and current position", "Firmware is at F2-R2.0",
                "H1-R2.22", "flex toward the antenna edge", "touch coordinates by 180 degrees",
            ),
            "README.ru.md": (
                "Роадмап прошивки и текущая позиция", "Прошивка находится на F2-R2.0",
                "H1-R2.22", "шлейфом к антенному", "touch-координаты", "180°",
            ),
        }
    ...
    def test_current_firmware_substep_is_visible_and_synchronized(self):
        ...
        self.assertEqual({"F2-R2.0"}, set(markers.values()))
        ...
        self.assertFalse(state["current_claims"]["f2_r2_target_rebaseline_started"])
