"""S3-only host regressions; no target configure/build or local SDK required."""
...
SCRIPT = ROOT / "targets/s3/verify_memory_config.py"
spec = importlib.util.spec_from_file_location("s3_memory_config", SCRIPT)
guard = importlib.util.module_from_spec(spec)
spec.loader.exec_module(guard)


class S3PsramConfigurationTests(unittest.TestCase):
    def setUp(self):
    ...
    def test_guard_runs_after_project_on_actual_sdkconfig_and_components(self):
        ...
        self.assertLess(text.index("project(leshy2_s3)"), text.index("verify_memory_config.py"))
        for token in ("LESHY2_S3_SDKCONFIG SDKCONFIG", "LESHY2_S3_COMPONENTS BUILD_COMPONENTS",
                      "LESHY2_S3_PYTHON PYTHON", '--sdkconfig "${LESHY2_S3_SDKCONFIG}"',
                      "--components ${LESHY2_S3_COMPONENTS}", "message(FATAL_ERROR"):
    ...
    def test_contract_or_target_defaults_cannot_silently_drop_ecc(self):
        for values in (self.canonical, self.defaults):
            with self.subTest(values=values):
                values["CONFIG_SPIRAM_ECC_ENABLE"] = "n"
                self.assertIn("CONFIG_SPIRAM_ECC_ENABLE", "\n".join(self.errors()))
                values["CONFIG_SPIRAM_ECC_ENABLE"] = "y"
    ...
    def test_wrong_target_numeric_speed_flash_and_hw_init_are_rejected(self):
        for key, value in (("CONFIG_IDF_TARGET", '"esp32c5"'),
    ...
    def test_partition_paths_resolve_from_their_own_roots(self):
        ...
        self.assertEqual([], self.errors())
        self.config["CONFIG_PARTITION_TABLE_CUSTOM_FILENAME"] = '"config/partitions_16m.csv"'
