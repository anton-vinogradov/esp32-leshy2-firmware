...
- **Прошивка / OTA.** C5 шьётся автономно через **свой USB-C** на столе. В поле **OTA идёт по самому линку SPI3** — S3 толкает образ C5 по линку (`OTA_BEGIN` / `OTA_DATA` / `OTA_END`), C5 стримит каждый фрагмент в неактивный OTA-раздел и только на `OTA_END` верифицирует и перезагружается. Обрыв линка посреди заливки безопасен: до `OTA_END` ничего не коммитится, рабочий образ нетронут, а S3 возобновляет с последнего заACKенного `offset` (или начинает заново). Моста прошивки S3→C5 по UART нет.
...
| `0x40` | `SLEEP` | — | парковать C5 (радио простаивают) |
| `0x41` | `WAKE` | — | |
...
| `0x52` | `OTA_DATA` | `offset:u32, bytes…` | один самоиндексируемый фрагмент: `REQ_ACK`, ключ — `offset`, ретрансмит пофрагментно по таймауту ACK; C5 стримит его прямо в неактивный OTA-раздел (в RAM не пересобирается). Не цепочка `MORE`. |
| `0x53` | `OTA_END` | — | верификация `sha256` + коммит → C5 ребутится в новый образ. **Коммит только на END:** обрыв линка до `OTA_END` оставляет рабочий образ нетронутым; S3 возобновляет с последнего заACKенного `offset` или перезапускает заливку. |
...
- **Dead-man на TX (safety).** Всякий раз, когда C5 передаёт (Wi-Fi deauth/flood, nRF24 TX / mousejack, ИК-посылка), ему нужен периодический `KEEPALIVE` (или любая команда) от S3. Если линк молчит `T_deadman`, C5 **сам гасит свой TX** — так что мёртвый S3 или порванный линк не могут оставить C5 передающим. Это протокольная половина safety-блокера «[STOP гасит весь TX](../README.ru.md#1-видение-и-рамки)»; `STOP_ALL` — мягкий путь, `C5_EN` низко — жёсткое убийство.
...
stateDiagram-v2
    ...
    UP --> SLEEP: SLEEP (радио простаивают)
    SLEEP --> BOOTING: WAKE
    UP --> UPDATING: OTA_END (верификация + коммит)
    UPDATING --> BOOTING: C5 ребутится в новый образ
```

Ребро `UPDATING` моделирует ожидаемое молчание после `OTA_END`: S3 пережидает самопрошивку-и-ребут C5, а не гонит на нём путь FAULT.
...
Каждый RF-TX-опкод — Wi-Fi, nRF24 и ИК — приходит в одно состояние `TX`, поэтому все они покрыты `STOP_ALL` и самогашением по `T_deadman`. `STOP_ALL` форсит `IDLE` (TX выкл) из любого состояния и обслуживается впереди всех прочих команд.

---

## 8. Версионирование и возможности

Байт `ver` гейтит весь формат. На загрузке C5 шлёт `HELLO {ver, caps}`; `caps` — битмап того, что эта сборка C5 реально умеет (5 ГГц-скан, снифф, deauth, flood, nRF24 скан / TX / mousejack, ИК посылка / обучение, OTA-по-линку, …). S3:
