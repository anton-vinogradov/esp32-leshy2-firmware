...
Hardware [`PIN-0002`](https://github.com/anton-vinogradov/esp32-leshy2/blob/main/docs/review/architecture/PIN-0002-zero-based-exact-pin-maps.md) completes exact pin/controller/recovery mapping for all three candidates. `SYN-2A/2B` have no safe generic GPIO reserve. `SYN-3A` adds RP2354 and retains five generic C5 GPIO plus two permanently service-reserved UART0 pins under [`DEC-0031`](https://github.com/anton-vinogradov/esp32-leshy2/blob/main/docs/review/decisions/DEC-0031-permanent-three-domain-development-access.md); hardware `FND-0038` corrects the former seven-generic-pin claim. `FND-0034` moves `SYN-2A` U214/GNSS to C5 without dropping capability. Hardware `FND-0037/REC-0001` corrects the C5 physical recovery selector to GPIO28-low with GPIO27-high plus CHIP_PU; GPIO26 is not the USB/UART Joint Download Boot 0 selector.
...
The active cross-repository gate is stage 4 component/BOM qualification. Hardware [`BOM-0001`](https://github.com/anton-vinogradov/esp32-leshy2/blob/main/docs/review/components/BOM-0001-stage4-component-evidence-register.md) is a reviewed complete register, not a component pass. Hardware [`BOM-0002`](https://github.com/anton-vinogradov/esp32-leshy2/blob/main/docs/review/components/BOM-0002-compute-clock-recovery-evidence.md) reviews the compute/clock/recovery facts. The owner accepted `IMP-0024/A` as [`DEC-0029`](https://github.com/anton-vinogradov/esp32-leshy2/blob/main/docs/review/decisions/DEC-0029-c5-v1.2-production-floor.md): production/release/qualification C5 requires ≥v1.2, with v1.0 restricted engineering-only; hardware `REV-0004C` reviews propagation. The owner then accepted `IMP-0025/A` as [`DEC-0030`](https://github.com/anton-vinogradov/esp32-leshy2/blob/main/docs/review/decisions/DEC-0030-vendored-critical-cad-libraries.md). Hardware `LIB-0001/REV-0004D/E` now review the repository-local C-001…005 symbols/footprints, provenance/licensing, CI pin/pad/hash gate and KiCad 10.0.5 parser checks; `FND-0036` is closed at CAD-representation level. The owner then chose permanent development access for every MCU in [`DEC-0031`](https://github.com/anton-vinogradov/esp32-leshy2/blob/main/docs/review/decisions/DEC-0031-permanent-three-domain-development-access.md); hardware `SVC-0001/REV-0004G` review the three-USB-C/DBG10/BOOT/RESET topology and first component targets, while CAD/AVL/mechanics/schematic and erased-image/multi-host HIL remain open. Firmware implementation remains unstarted and must consume these exact hardware contracts rather than infer hardware from legacy sources.
