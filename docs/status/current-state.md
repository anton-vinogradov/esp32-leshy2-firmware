...
- `FND-0028`: physical ownership of the three full-function nRF24 radios is reopened; S3 shared-SPI and C5+SDIO were compared and await the `IMP-0021` decision.
...
The 3×nRF24 capability audit passed `REV-0002T`/`REV-0002U`: [`REQ-N24-0001`](https://github.com/anton-vinogradov/esp32-leshy2/blob/main/docs/review/requirements/REQ-N24-0001-three-nrf24-raw-2g4.md) preserves three simultaneous full-function radios and accepted [`DEC-0019`](https://github.com/anton-vinogradov/esp32-leshy2/blob/main/docs/review/decisions/DEC-0019-calibrated-rpd-three-antenna-hunt.md), a calibrated binary RPD hit-rate sector comparison that is never RSSI/dBm/bearing/VSWR. After the owner clarified that nRF24 must be full-function, physical ownership was reopened. `REV-0002Z` and [`AUD-0003`](https://github.com/anton-vinogradov/esp32-leshy2/blob/main/docs/review/audits/AUD-0003-three-nrf24-owner-placement.md) show that S3 already has physical nets and two GP-SPI controllers, while C5 requires moving the inter-MCU link to SDIO, a tight pin budget, and raw-frame IPC. **⚠️ Proposal [`IMP-0021/A`](https://github.com/anton-vinogradov/esp32-leshy2/blob/main/docs/review/improvements/IMP-0021-s3-owns-three-full-function-nrf24.md)** recommends S3 shared SPI with an independent reset-safe CE latch; it is not accepted and requires pin/recovery/worst-case latency-loss HIL. `FND-0019` and `FND-0021` remain implementation gates.
