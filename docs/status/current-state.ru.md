...
Каноническая таблица стадий — [`docs/review/stages.md`](https://github.com/anton-vinogradov/esp32-leshy2/blob/main/docs/review/stages.md). Firmware ownership, transport, safety, update и failure boundaries приняты в [`ARC-0001`](../architecture/ARC-0001-three-domain-runtime-contract.md); toolchain, конкретное wire encoding, структура каталогов и реализация остаются следующей работой.
...
Zero-based метод сначала фиксировал только продуктовые роли. `DEC-0028` теперь разрешает весь target: RP2354A напрямую владеет тремя nRF24, CC1101 и voice real-time control; 1-bit SDIO и SPI+alert приняты как links. [`ARC-0001`](../architecture/ARC-0001-three-domain-runtime-contract.md) распространяет runtime contract.

Hardware [`SYN-0001`](https://github.com/anton-vinogradov/esp32-leshy2/blob/main/docs/review/architecture/SYN-0001-zero-based-whole-device-candidates.md) сравнил три полных zero-based placement. `REV-0003N/3O` проверили их без выбора winner; последующий атомарный package выбрал трёхдоменный deterministic RP2354A A4 design в `DEC-0028`.

Hardware [`PIN-0002`](https://github.com/anton-vinogradov/esp32-leshy2/blob/main/docs/review/architecture/PIN-0002-zero-based-exact-pin-maps.md) провёл exact pin/controller/recovery mapping всех трёх candidates. `SYN-2A/2B` не имеют safe generic GPIO reserve. `SYN-3A` добавляет RP2354 и оставляет пять generic C5 GPIO плюс две постоянно зарезервированные под service UART0 ноги по [`DEC-0031`](https://github.com/anton-vinogradov/esp32-leshy2/blob/main/docs/review/decisions/DEC-0031-permanent-three-domain-development-access.md); hardware `FND-0038` исправляет прежнее утверждение о семи generic pins. `FND-0034` перенёс U214/GNSS `SYN-2A` на C5 без потери функций. Hardware `FND-0037/REC-0001` исправляет physical recovery selector C5 на GPIO28-low при GPIO27-high плюс CHIP_PU; GPIO26 не является selector USB/UART Joint Download Boot 0.
...
Hardware [`RFQ-0001`](https://github.com/anton-vinogradov/esp32-leshy2/blob/main/docs/review/architecture/RFQ-0001-zero-based-rf-zoning-coexistence.md) прошёл `REV-0003R`. Геометрия антенн и RF paths одинаковы, обязательный three-nRF PRX сохранён, cross-domain receive пары начинаются как qualification-only, все TX пары — как запрещённые. Авторизация не отменяет shielded/conducted и spectrum gate.

Hardware [`CST-0001`](https://github.com/anton-vinogradov/esp32-leshy2/blob/main/docs/review/architecture/CST-0001-dated-candidate-cost-burden.md) прошёл `REV-0003S`. `2B` — recurring-BOM minimum, `2A` — implementation-burden minimum, `3A` — margin maximum примерно с $1.10 conservative historical midpoint premium над `2A` и третьим signed target. Hardware `FND-0035` исправляет старый вывод о stock RP2354A меньше 500 по exact `SC1511-A4`; production quotes, traceability и assembly proof остаются открыты.
...
Активный межрепозиторный gate — этап 4 qualification компонентов/BOM. Hardware [`BOM-0001`](https://github.com/anton-vinogradov/esp32-leshy2/blob/main/docs/review/components/BOM-0001-stage4-component-evidence-register.md) является полным проверенным register, а не component pass. Hardware [`BOM-0002`](https://github.com/anton-vinogradov/esp32-leshy2/blob/main/docs/review/components/BOM-0002-compute-clock-recovery-evidence.md) проводит ревью compute/clock/recovery facts. Владелец принял `IMP-0024/A` как [`DEC-0029`](https://github.com/anton-vinogradov/esp32-leshy2/blob/main/docs/review/decisions/DEC-0029-c5-v1.2-production-floor.md): production/release/qualification C5 требует ≥v1.2, v1.0 остаётся restricted engineering-only; hardware `REV-0004C` проверяет propagation. Затем владелец принял `IMP-0025/A` как [`DEC-0030`](https://github.com/anton-vinogradov/esp32-leshy2/blob/main/docs/review/decisions/DEC-0030-vendored-critical-cad-libraries.md). Hardware `LIB-0001/REV-0004D/E` теперь проверяют repository-local symbols/footprints C-001…005, provenance/licensing, CI pin/pad/hash gate и parser checks KiCad 10.0.5; `FND-0036` закрыт на уровне CAD representation. Затем владелец выбрал постоянный development access для каждого MCU в [`DEC-0031`](https://github.com/anton-vinogradov/esp32-leshy2/blob/main/docs/review/decisions/DEC-0031-permanent-three-domain-development-access.md); hardware `SVC-0001/REV-0004G` проверяют topology трёх USB-C/DBG10/BOOT/RESET и первые component targets, а CAD/AVL/mechanics/schematic и erased-image/multi-host HIL остаются открыты. Firmware implementation не начата и обязана потреблять эти exact hardware contracts, а не выводить hardware из legacy sources.
