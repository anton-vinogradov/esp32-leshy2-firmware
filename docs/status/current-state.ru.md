...
Prerequisite audit 3×nRF24 получил статус **«Проведено ревью»** в `REV-0002T`, а финальное распространение в `REV-0002U` дало [`REQ-N24-0001`](https://github.com/anton-vinogradov/esp32-leshy2/blob/main/docs/review/requirements/REQ-N24-0001-three-nrf24-raw-2g4.md) статус **«Проведено ревью»**. Владелец принял `IMP-0016/A` как [`DEC-0019`](https://github.com/anton-vinogradov/esp32-leshy2/blob/main/docs/review/decisions/DEC-0019-calibrated-rpd-three-antenna-hunt.md): три одновременных тракта дают calibrated binary RPD hit-rate sector comparison с sampling/calibration metadata, но никогда не RSSI/dBm/bearing/VSWR; real-power hardware остаётся отдельным будущим решением. `FND-0020` закрыт на requirement-level. Текущее железо остаётся S3-routed, а активного C5 driver/security state machine/HIL нет, поэтому `FND-0019` и `FND-0021` открыты. Passive ESB/MouseJack discovery, sensitive KeySniffer capture, active injection, address brute-force и interference tests сохраняют раздельные gates; interference/carrier sweep допустимы только conducted/RF-shielded `BOTH`, а GPL references не считаются MIT-reusable code. **⚠️ Предложение [`IMP-0017`](https://github.com/anton-vinogradov/esp32-leshy2/blob/main/docs/review/improvements/IMP-0017-native-ble-plus-nrf24-compatibility.md)** отложено до BLE-owner review.
...
Native BLE prerequisite audit [`REV-0002X`](https://github.com/anton-vinogradov/esp32-leshy2/blob/main/docs/review/reviews/REV-0002X-ble-prerequisites.md) получил статус **«Проведено ревью пререквизитов»** и сформировал draft [`REQ-BLE-0001`](https://github.com/anton-vinogradov/esp32-leshy2/blob/main/docs/review/requirements/REQ-BLE-0001-native-ble-and-security.md), который остаётся **«На ревью»** до ownership. Оба ESP поддерживают legacy ordinary BLE baseline, но C5 BLE конфликтует за radio-time с принятым Thread/Zigbee; **⚠️ Предложение [`IMP-0019`](https://github.com/anton-vinogradov/esp32-leshy2/blob/main/docs/review/improvements/IMP-0019-s3-primary-native-ble-owner.md)** рекомендует S3 как единственного baseline owner, C5 BLE default-off и nRF24 только для limited compatibility/research. Native scan не объявлен connection sniffer/идентификатором/дальномером (`FND-0026`), vendor/emulation/attack claims получили corpus, rights и трёхуровневые gates (`FND-0027`). Dedicated nRF52 connection sniffer остаётся отдельным `IMP-0004`. Найденный «лишний» Bluetooth Mesh не добавлен в target: **⚠️ Предложение [`IMP-0020`](https://github.com/anton-vinogradov/esp32-leshy2/blob/main/docs/review/improvements/IMP-0020-ordinary-ble-mesh-scope.md)** отложено до решения владельца BLE.
