...
- `FND-0028`: три полные static nRF ownership maps сравнены. По `DEC-0026` они являются входами единого package; отдельного owner decision больше нет.
...
Capability-аудит 3×nRF24 прошёл `REV-0002T`/`REV-0002U`: [`REQ-N24-0001`](https://github.com/anton-vinogradov/esp32-leshy2/blob/main/docs/review/requirements/REQ-N24-0001-three-nrf24-raw-2g4.md) сохраняет три одновременных полнофункциональных radio и принятый [`DEC-0019`](https://github.com/anton-vinogradov/esp32-leshy2/blob/main/docs/review/decisions/DEC-0019-calibrated-rpd-three-antenna-hunt.md) — calibrated binary RPD hit-rate sector comparison, никогда не RSSI/dBm/bearing/VSWR. Physical owner остаётся открытым внутри общего package. `REV-0002Z`/`AUD-0003` и **⚠️ `IMP-0021/A`** дают сравнительный вход, но по [`DEC-0026`](https://github.com/anton-vinogradov/esp32-leshy2/blob/main/docs/review/decisions/DEC-0026-atomic-integrated-architecture-acceptance.md) owner/transport отдельно не принимаются. `FND-0019`/`FND-0021` остаются implementation gates.
...
[`ADR-0001`](https://github.com/anton-vinogradov/esp32-leshy2/blob/main/docs/review/architecture/ADR-0001-stage3-dependency-register.md) сопоставил весь stage-3 graph с demand/hard-fail rows и получил статус **«Проведено ревью»** в `REV-0003I`. Owner/transport/memory, matrix/audio/recovery, external ports/power и actual-TX/coexistence нельзя закрывать независимо.
...
[`DEC-0026`](https://github.com/anton-vinogradov/esp32-leshy2/blob/main/docs/review/decisions/DEC-0026-atomic-integrated-architecture-acceptance.md) требует атомарной приёмки всей архитектуры. Corrected matrix/`U14`, **⚠️ [`IMP-0021/A`](https://github.com/anton-vinogradov/esp32-leshy2/blob/main/docs/review/improvements/IMP-0021-s3-owns-three-full-function-nrf24.md)**, memory, transport, exact pins, recovery, STOP/TX-state, coexistence и cost больше не считаются отдельными вопросами. `LAY-S3` — первый synthesis candidate, но не target до сведения всего package и его kill gates.
