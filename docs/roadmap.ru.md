...
> H3-R2.0 и H3-R2.1 проведены ревью, а полный workstream H3-R2.2 проходит 14 сценариев startup/reset/recovery, 7 316 переходов USB/pack/DPM/brownout/source-loss, пять запусков защищённых шин, четыре load-step envelope и десять watchdog/fault-display cases без небезопасного допуска или автоматического перезапуска. [Аналоговый результат H3-R2.3](https://github.com/anton-vinogradov/esp32-leshy2/blob/main/docs/analog-electrical-verification.ru.md) проведён ревью; текущая аппаратная точка — H3-R2.4 для цифровых уровней, таймингов, нагрузки и прямого i8080-8. Точные firmware-контракты последовательностей, handover и watchdog/fault-display импортированы в `config/h3_r2_transition_contract.json`, `config/h3_r2_handover_contract.json` и `config/h3_r2_inrush_watchdog_contract.json`. H2-R2.1.1 провёл ревью трёх native-проектов, 23 sheets, шести владельцев доменов, 239 точных MPN-групп и 1 195 позиций, а H2-R2.1.2 — 234 board groups, пять non-PCBA groups и 1 656 логических контактов. Более ранние live route FSUSB42MUX/C11355, точный тракт service-VBUS detector/latch/release и граница TCA9803DGKR/C2687966 Pack/Safety также прошли ревью. Точной импортированной pin/config
...
| Пересечение с железом | ▶️ H0-R2, H1-R2.37, H2-R2.1.5, H3-R2.1, полный [результат H3-R2.2](https://github.com/anton-vinogradov/esp32-leshy2/blob/main/docs/power-transition-result.ru.md) и [аналоговый результат H3-R2.3](https://github.com/anton-vinogradov/esp32-leshy2/blob/main/docs/analog-electrical-verification.ru.md) проведены ревью; H3-R2.4 с цифровыми уровнями, таймингами, нагрузкой и прямым i8080-8 — текущая точка. H3-R2.2 проходит 14 сценариев startup/reset/recovery, 7 316 source-transition cases, пять запусков, четыре load-step envelope и десять fault cases без небезопасного допуска или автоматического перезапуска. Три native-проекта KiCad содержат 1 185 устанавливаемых позиций, 4 323 физических pins и 823 канонических nets без замечаний ERC; сведены шесть доменов, 173 строки контроллеров, 52 межпроектные и 238 межлистовых nets. Импортированной exact pin/config authority остаётся H1-R2.31; Pack/Safety идёт Hub GPIO42/43 → TCA9803DGKR → AON MSPM0 и не зависит от hard-kill. У M1 девять настоящих NC, latched `FAULT_KILL` на M1-35 и отдельный reset S3 на M1-36 |
