...
> [Текущий H3](https://github.com/anton-vinogradov/esp32-leshy2/blob/main/docs/h3-r2-acceptance.ru.md)
> имеет статус `review_required`; исправленные [пути напряжения шин](https://github.com/anton-vinogradov/esp32-leshy2/blob/main/docs/power-rail-margins.ru.md)
> остаются предварительной моделью, а не подтверждением установленной схемы MAIN/AON.
> Все семь импортов H3 в прошивку сохраняют эту границу: `evidence_status=provisional`.
> `make h3-current-scope-review` (в составе `make test`) проверяет полную цепочку источников;
> разрешения на производство, питание от аккумулятора и исполнение на целевых контроллерах остаются false.
...
> Одобренное пользователем обновление 9 сентября унифицирует четыре USB-C
> разъёма как `GCT USB4105-GF-A`: удалено одно уникальное определение JAE и его
> 17 контактов, а не установленные endpoints. 1 208 экземпляров / 4 305 физических
> выводов, GPIO/API, 13 C/H-файлов BSP и прежний результат 12 сборок не изменились.
> Последующее исправление источников C5 от 14 сентября добавляет один NAND и один
> bypass-конденсатор: исходная топология TS3USB221ERSER/LV20A/NX3008NBKS проходит
> точную проверку, но программный менеджер не реализован и все четыре флага
> квалификации C5 остаются false. Физический KILL не меняется. Меняются только четыре
> описания endpoints C5/Hub BSP; GPIO/API и прежние сборки не переквалифицируются.
...
> ориентацию физических контактов Cap, точное крепление держателя/энкодера и акустический доступ.
> Карта Cap и GPIO/API прошивки не менялись; исправление в плоскости не означает разрешение сборки.
> [Исправление восстановления S3 от 14 сентября](https://github.com/anton-vinogradov/esp32-leshy2/blob/main/docs/service-recovery.ru.md)
> уже меняет GPIO: UART0 TX/RX используют GPIO43/44, Hub D2/D3 — GPIO7/8.
> Меняются `s3_bsp.h`, `s3_bsp.c` и `hub_rp_bsp.c`; прежние результаты сборок,
> включая PSRAM-пробу S3, не квалифицируют текущие исходники или восстановление.
> Концепт H1 из 223 тел не является текущей геометрией native PCB или разрешением сборки.
...
> но историческое ревью H3 не закрывает текущий результат `review_required`, указанный выше.
> [Ревью H4](https://github.com/anton-vinogradov/esp32-leshy2/blob/main/docs/h4-r2-acceptance.ru.md)
> и [отчёт маршрутов H5](https://github.com/anton-vinogradov/esp32-leshy2/blob/main/docs/h5-r2-current-route.ru.md)
> сохраняют границы своих проверенных исходных версий. Отчёт H5 содержит текущий
> закупочный охват и явные условия допуска заказа, включая `WBC16-1TLC`;
> он не заменяет новую проверку непосредственно перед закупкой.
> Сейчас выполняется H6.0.3-R1: после исправления локальности компонентов разводка
> двух плат 80 × 150 мм проходит повторную квалификацию. Точные числа элементов
> меди, связность, результаты DRC и изображения поддерживаются в
> [актуальном аппаратном срезе разводки](https://github.com/anton-vinogradov/esp32-leshy2/blob/main/docs/h6-r2-current-routing.ru.md).
> Прежние результаты трассировки являются историческими и не подтверждают завершение
> каких-либо текущих классов цепей. H2-R2.1.1 провёл ревью двух native-проектов,
> 22 sheets, шести владельцев доменов, 252 точных component-groups и 1 220 позиций.
> H2-R2.1.2 проверил 246 board groups, шесть non-PCBA groups и 1 615 контактов
> уникальных определений. Точной импортированной pin/config
...
> отвечает за две нативные шестислойные платы; размещение текущих 1 210 экземпляров
> требует собственного обновлённого доказательства. [Точный механический стек H6](https://github.com/anton-vinogradov/esp32-leshy2/blob/main/docs/h6-r2-mechanical-stack.ru.md)
> фиксирует 20-мм нейлоновый крепёж M2.5, захваченные гайки, 11-мм упоры и
> независимые захваты PCB без нагрузки на M1. Проверки размещения, механики и кабелей повторяются при затрагивающем их изменении геометрии; [правила трассировки H6](https://github.com/anton-vinogradov/esp32-leshy2/blob/main/docs/h6-r2-routing-policy.ru.md)
> управляют текущей разводкой. Locality-first
> размещение двух плат, фильтр Airband, точный дисплей и все корпуса U219 проходят текущие
> структурные проверки. Расчётная нагрузка 3,046 А сама по себе не доказывает
> достаточный запас ограничения тока. Концептуальный реестр из 223 тел также содержит все восемь точных TX-детекторов,
> пять coupler и восемь ограниченных локальных evidence-островов; принятые AD8314 и точная упаковочная версия Hirose U.FL не меняют
> видимый прошивке контракт. Все текущие группы топ-20 сохранены, а отдельные `ANT-433-CW-QW-SMA` постоянно закреплены за SUB-GHz и UHF VOICE и не становятся разделяемой прошивкой антенной. Бортовой видеоприёмник, декодер, разъём и требующая ручной пайки зона
...
> сгенерированы. Текущий R2 после C5 ECO 14 сентября содержит 1 210 устанавливаемых позиций
> и 4 321 физический вывод в двух native-проектах KiCad: 4 085 подключены,
> 236 явно не подключены, 789 глобальных канонических сетей. Эти счётчики схемы
> не доказывают связность разведённой меди, наличие источников у всех шин или
> отсутствие конфликтующих выходов; электрическая и физическая проверки остаются
> обязательными до производственного выпуска.
> Исправления физических выводов 7 сентября вернули питание QSPI обоим RP2354 и исправили
> три распиновки компонентов: подключены 4 070 физических pins, GPIO/API прошивки не изменились.
> Расширенное [электрическое ревью](https://github.com/anton-vinogradov/esp32-leshy2/blob/main/docs/h6-r2-electrical-semantics.ru.md)
> добавило исходные исправления TPS3839/TPD2EUSB30A/B3S/FH34; обновлённые контракты
> H2/H3 сохраняют код BSP и не означают завершённую электрическую проверку железа.
> Текущий H3 условно моделирует установленные TPS566231P и R67 = 1,65 кОм,
> но не квалифицирует ячейку питания. Фактические VIN, пульсации, падение напряжения
> за защитой, запас PGTH, применимость RILIM для AON eFuse и запуск остаются открытыми.
> Детали питания не менялись; синхронизация источников не доказывает запуск под питанием.
> H2-R2.1.4 свёл шесть доменов,
> 173 строки контроллеров, 34 межпроектные и 227 межлистовых nets; sync gate
> результата H2-R2.1.5 открыт. Теперь H3 фиксирует эти входы. Разводка
> H6.0.3-R1 продолжается, заказ не разрешён.

Последняя сверка текста об аппаратном статусе: **14 сентября 2026 года**. Это собственный роадмап
...
quad PSRAM, 40 МГц, ECC выключен. На input `34ec149` прошли один чистый C5 DEBUG
configure и одна сборка: проверены семь artifacts, `esp_psram_init` слинкован
([ограниченный receipt](../config/f2_r2_c5_build_probe.json)). Это не доказывает
runtime-инициализацию памяти или доступный объём. Сохранённое подтверждение
двенадцати сборок не менялось; квалификация всей текущей матрицы, побайтовая
воспроизводимость и восстановление остаются открытыми.
...
Текущая граница — `F2-R2.5`. Исторический результат F2-R2.4 на `c8e349b`
прошёл свою locked 12-job matrix, проверил все 60 artifacts и 16 maps и прошёл
все 16 size gates; текущие входы он не квалифицирует. Далее нужно собрать locked
matrix на текущих входах и проверить два побайтно идентичных чистых прохода,
сравнив каждый объявленный artifact и сохранив исторические receipts.
Runtime и S3 QEMU остаются gates F3-R2; F2-R2.4 не заявляет emulator,
development-board или hardware execution.
