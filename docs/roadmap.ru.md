...
> [Глобальный итог H3-R2](https://github.com/anton-vinogradov/esp32-leshy2/blob/main/docs/h3-r2-acceptance.ru.md), [глобальный итог H4-R2](https://github.com/anton-vinogradov/esp32-leshy2/blob/main/docs/h4-r2-acceptance.ru.md) и [актуальный итог маршрутов H5-R2](https://github.com/anton-vinogradov/esp32-leshy2/blob/main/docs/h5-r2-current-route.ru.md) проведены ревью; H5 контролирует все 249 закупаемых групп / 1 216 изделий без неназначенных маршрутов и с одним явным order-time sourcing gate `WBC16-1TLC`. H6.0.1 завершил placement, механический стек и пять сервисных коридоров microcoax; сейчас выполняется H6.0.3-R1: обе платы имеют размер 80 x 150 мм, все 1 208 позиций остаются зафиксированы без жёстких конфликтов, а [актуальный срез разводки](https://github.com/anton-vinogradov/esp32-leshy2/blob/main/docs/h6-r2-current-routing.ru.md) связывает 5 305 чистых по DRC элементов меди с hash плат — 715 физических соединений замкнуто, 2 550 осталось в новой геометрии с 5-мм коридором внутренних сторон; UI analogue/audio/sense завершён; после 107 новых чистых соединений на RF/power в этом классе осталось 20. H2-R2.1.1 провёл ревью двух native-проектов, 22 sheets, шести владельцев доменов, 251 точной component-group и 1 218 позиций. Точной импортированной pin/config
...
> независимые захваты PCB без нагрузки на M1. H6.0.1 завершена; принятый [срез трассировки H6.0.2](https://github.com/anton-vinogradov/esp32-leshy2/blob/main/docs/h6-r2-routing-policy.ru.md)
> замыкает все 366/366 соединений в 165 сетях `GENERAL_CONTROL` и 12 сетях `OSCILLATOR` с нулём нарушений DRC, а safety-control и аналоговые/audio/sense-сети остаются текущей работой. Locality-first
...
> 4 064 подключённых endpoints, 238 явных no-connects и 789 глобальных канонических / 823 локальных для плат nets; оба проекта
> проходят KiCad ERC с нулём ошибок и предупреждений. H2-R2.1.4 свёл шесть доменов,
> 173 строки контроллеров, 34 межпроектные и 228 межлистовых nets; sync gate
> результата H2-R2.1.5 открыт. Теперь H3 фиксирует эти входы. Hardware H6.0.1
> завершена, срез `GENERAL_CONTROL` + `OSCILLATOR` проходит; критичная ручная трассировка
> H6.0.2 продолжается, заказ не разрешён.
...
| Пересечение с железом | ✅ H0-R2 — актуальный итог маршрутов H5-R2 проведены ревью; H5 контролирует все 249 закупаемых групп / 1 216 изделий без неназначенных маршрутов и с одним явным order-time sourcing gate `WBC16-1TLC`. Hardware H6.0.1 проходит; срез `GENERAL_CONTROL` + `OSCILLATOR` в H6.0.2 замыкает 366/366 соединений с нулём нарушений DRC, а safety-control, аналоговая/audio/sense-трассировка и native net parity продолжаются. H3-R2.4 доказывает прямой i8080-8 ровно 20 МГц, однозначное владение USB и паритет 80/80 контактов M1. Все 51 physical-остаток и отдельное обязательство F5/F6 по i8080 остаются открытыми у точных владельцев. |
