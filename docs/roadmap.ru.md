...
> 4 067 подключённых endpoints, 256 явных no-connects и 826 канонических nets; все три проекта
...
| Пересечение с железом | ▶️ H0-R2, H1-R2.37, H2-R2.1.5, H3-R2.1 и полный [результат H3-R2.2](https://github.com/anton-vinogradov/esp32-leshy2/blob/main/docs/power-transition-result.ru.md) проведены ревью; H3-R2.3 с analog corners — текущая точка. H3-R2.2 проходит 14 сценариев startup/reset/recovery, 7 316 source-transition cases, пять запусков, четыре load-step envelope и десять fault cases без небезопасного допуска или автоматического перезапуска. Три native-проекта KiCad содержат 1 187 устанавливаемых позиций, 4 327 физических pins и 826 канонических nets без замечаний ERC; сведены шесть доменов, 173 строки контроллеров, 52 межпроектные и 238 межлистовых nets. Импортированной exact pin/config authority остаётся H1-R2.31; Pack/Safety идёт Hub GPIO42/43 → TCA9803DGKR → AON MSPM0 и не зависит от hard-kill. У M1 девять настоящих NC, latched `FAULT_KILL` на M1-35 и отдельный reset S3 на M1-36 |
