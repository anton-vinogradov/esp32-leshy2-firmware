...
| Пересечение с железом | ▶️ H0-R2, H1-R2.37 и H2-R2.1.5 прошли ревью; H3-R2.0.1…H3-R2.1.2 проведены ревью, H3-R2.1.3 расчёт запасов шин — текущая точка. H3-R2.1.2 явно связывает 596 устанавливаемых питаемых экземпляров и шесть внешних нагрузок без скрытого aggregate. 2 266 разрешённых состояния включают U214/U219, Airband и все сочетания трёх nRF. Три native-проекта KiCad содержат 1 187 устанавливаемых позиций, 4 327 физических pins и 827 канонических nets без замечаний ERC; сведены шесть доменов, 173 строки контроллеров, 50 межпроектных и 233 межлистовых nets. Импортированной exact pin/config authority остаётся H1-R2.31; Pack/Safety идёт Hub GPIO42/43 → TCA9803DGKR → AON MSPM0 и не зависит от hard-kill. Карты двух RP, десять SMA 5+5, локальный S3 i8080-8 24 МГц, 11 настоящих NC M1 и принятая модель 226 тел не изменились |
