...
> production ECAD-схемы hardware R2, которой пока нет. Ни один target-образ и
...
| HIL и release | 🔒 Ожидают аппаратный прототип R9 |
...
| **F10. HIL и системная квалификация** | 🔒 Ожидает F4–F9 и hardware R9 | Автоматизированные тесты на собранном прототипе, fault injection, RF/power/thermal/endurance | Пройдены реальные transports/peripherals, 3×nRF concurrency, quiet-state, watchdog, thermal, brownout, update interruption, длительная работа 24–48 ч и безопасное восстановление |
| **F11. Firmware release** | 🔒 Ожидает F10 и hardware R10 | Воспроизводимые образы, installer, release notes, recovery kit и совместимый тег | Ноль blocker; target binaries воспроизводимы и подписаны; SBOM/licenses/tests опубликованы; сайт описывает реализованные возможности; firmware tag совместим с hardware release |
...
Текущая граница — F2. До hardware R2 можно подготовить только воспроизводимую
