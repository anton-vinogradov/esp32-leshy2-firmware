...
> Полный [глобальный итог H3-R2](https://github.com/anton-vinogradov/esp32-leshy2/blob/main/docs/h3-r2-acceptance.ru.md) проведён ревью: 20 текущих artifacts и 129 записанных source hashes сведены без mismatch или открытого аналитического finding, а 51 physical-only строка остаётся назначенной H5/H6/H8. Точные firmware-контракты последовательностей, handover, watchdog/fault-display, цифровых интерфейсов, RF/coexistence, thermal/fault и [H3 acceptance](../config/h3_r2_acceptance.json) импортированы fail-closed. H4-R2.0.1 фиксирует 24 точных текущих join-inputs при совпадении всех cross-repository H3 hashes; текущая аппаратная точка — H4-R2.0.2 для сверки видимых железу firmware-контрактов. H2-R2.1.1 провёл ревью трёх native-проектов, 23 sheets, шести владельцев доменов, 240 точных MPN-групп и 1 195 позиций, а H2-R2.1.2 — 234 board groups, шесть non-PCBA groups и 1 658 логических контактов. Более ранние live route FSUSB42MUX/C11355, точный тракт service-VBUS detector/latch/release и граница TCA9803DGKR/C2687966 Pack/Safety также прошли ревью. Точной импортированной pin/config
...
Последняя сверка статуса: **1 сентября 2026 года**. Это собственный роадмап
...
| Пересечение с железом | ▶️ H0-R2, H1-R2.37, H2-R2.1.5 и полный [итог H3-R2](https://github.com/anton-vinogradov/esp32-leshy2/blob/main/docs/h3-r2-acceptance.ru.md) проведены ревью; H4-R2.0.1 фиксирует 24 объединённых inputs, а H4-R2.0.2 сверки контрактов — текущая точка. H3-R2.4 доказывает прямой i8080-8 ровно 20 МГц, положительные запасы уровней/таймингов, однозначное владение USB и паритет 80/80 контактов M1; H3-R2.5 импортирует десять постоянных RF-трактов, 13 quiet contracts и все четыре сочетания ролей трёх nRF24; H3-R2.6 импортирует 56 thermal-профилей, 30 single-fault сценариев и extended-operation policy без обещания автономности. H3-R2.7 импортирует ноль аналитических findings, 51 назначенный физический остаток и одно явное обязательство F5/F6 по i8080. Три native-проекта KiCad содержат 1 185 устанавливаемых позиций, 4 323 физических pins и 823 канонических nets без замечаний ERC; сведены шесть доменов, 173 строки контроллеров, 52 межпроектные и 238 межлистовых nets. Импортированной exact pin/config authority остаётся H1-R2.31; Pack/Safety идёт Hub GPIO42/43 → TCA9803DGKR → AON MSPM0 и не зависит от hard-kill. У M1 девять настоящих NC, latched `FAULT_KILL` на M1-35 и отдельный reset S3 на M1-36 |
