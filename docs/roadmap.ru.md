...
> Железо находится на H1-R2.20; актуальное locality-first размещение двух плат,
...
> совпадающего семейства; аппаратный аудит использует консервативный резерв
...
> блокер H1 — формальная эквивалентность K331↔SP331RX или собственный пакет AKK,
> плюс maximum Z/допуски, рекомендуемые land/paste и packaging/soldering/reflow;
> он же открывает Consigned Parts application.
> Финальный DFM и дополнительный фабричный function test относятся к H5/H6/H7;
> квалификация RF/video и запасной антенны остаётся работой H3/H5/H6/H8.
> Контролируемые корпус/посадка `AWM666V RX` входят в тот же резерв, но семь
> каналов 5725–5875 МГц и 0 точных результатов JLCPCB оставляют его деградированным fallback.
...
| Пересечение с железом | ▶️ H0-R2 проведено ревью, H1-R2.20 сейчас; стабильная внешняя шелкография указывает роль каждой платы, `R2-EVT1` и `REV A`, а `H1-R2.xx` остаётся только в документации; десять SMA разделены 5+5 и имеют проверенную на пересечения пользовательскую шелкографию, вертикальный задний FPV MMCX не имеет хвоста в межплатный просвет, прямые i8080-8 32 МГц и camera RX остаются локальны S3, TVP5150 остаётся спереди, а точный 80-контактный M1 несёт один CVBS, 14 настоящих NC-резервов и не принимает механическую нагрузку корпуса; K331 RSSI официально NC, поддерживаемое свидетельство — decoder lock; официальный SP331RX контролирует номинальные XY/оси контактов семейства-кандидата, а формальная эквивалентность K331 и остаток production/assembly evidence блокируют H1; Consigned Parts/DFM/function-test следуют в H5/H6/H7, а физическое RF-доказательство остаётся у H3/H5/H6/H8 |
