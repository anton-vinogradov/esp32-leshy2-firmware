...
Исправлена исходная причина отсутствия PSRAM S3: `esp_psram` теперь является
явной зависимостью компонента, а configure-time guard проверяет полученную
конфигурацию по прежнему контракту flash 16 МиБ / octal PSRAM 8 МиБ
(80 МГц, инициализация при старте, ECC; после ECC адресуемы 7,5 МиБ).
Свежий clean configure probe пока не выполнен; сохранённая сборка PSRAM не включает.
Инициализация, доступный объём и self-test памяти остаются недоказанными до
runtime-проверки перед запуском дисплея.
