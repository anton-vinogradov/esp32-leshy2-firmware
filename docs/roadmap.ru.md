...
> H2-R2.0.3 после ревью live route FSUSB42MUX/C11355 как H2-R2.0.1 и точного тракта service-VBUS detector/latch/release как H2-R2.0.2. Точной импортированной pin/config
...
> сгенерированы. Native R2 ECAD/KiCad ещё не начинался, заказ не разрешён.
...
| Authority R2 и production H2 | 🔒 [Fail-closed gate](../config/r2_h2_sync_gate.json): точные рабочие карты двух RP, C5 4-bit mux и service-VBUS detector/latch/release импортированы как pre-H2 authority, но сохранённый H2.0.3 — исторический R1 и не авторизует R2; открыть только после закрытия Pack/Safety powered-off-Ioff и six-domain H2 export |
