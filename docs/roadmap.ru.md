...
> 4 063 подключённых endpoints, 260 явных no-connects и 827 канонических nets; все три проекта
> проходят KiCad ERC с нулём ошибок и предупреждений. H2-R2.1.4 свёл шесть доменов,
> 173 строки контроллеров, 50 межпроектных и 233 межлистовых nets; sync gate
...
| Пересечение с железом | ▶️ H0-R2, H1-R2.37, H2-R2.1.5 и весь DC/source-workstream H3-R2.1 прошли ревью; H3-R2.2.1 с упорядоченными startup/shutdown/reset/recovery — текущая точка. H3-R2.1 сводит 618 нагрузок, 224 rail-профиля и 2 266 состояний в 15 проходящих cross-checks. Максимальный ток pack — 3,516 А против границы 8 А; 9 В × 3 А и 15 В × 2 А запускают любой профиль, 14 слишком тяжёлых USB-only состояний на 5 В × 3 А отклоняются. Электрический угол 3V3_MAIN — 3,046 А против границы 3,2 А. Три native-проекта KiCad содержат 1 187 устанавливаемых позиций, 4 327 физических pins и 827 канонических nets без замечаний ERC; сведены шесть доменов, 173 строки контроллеров, 50 межпроектных и 233 межлистовых nets. Импортированной exact pin/config authority остаётся H1-R2.31; Pack/Safety идёт Hub GPIO42/43 → TCA9803DGKR → AON MSPM0 и не зависит от hard-kill. Карты двух RP, десять SMA 5+5, локальный S3 i8080-8 24 МГц, 11 настоящих NC M1 и принятая модель 226 тел не изменились |
