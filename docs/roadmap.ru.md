...
> H3-R2.0.1 зафиксировал стабильные входы верификации, H3-R2.0.2 провёл ревью provenance всех 242 групп и 1 187 устанавливаемых позиций, H3-R2.0.3 зафиксировал девять методов и двенадцать pass/fail rules, H3-R2.1.1 провёл ревью 2 266 разрешённых power states, H3-R2.1.2 — явной привязки всех нагрузок, H3-R2.1.3 — 224 проходящих профилей четырёх шин, H3-R2.1.4 — всех 75 source/pack-строк и безопасного допуска 2 266 состояний, а H3-R2.1.5 закрыл проведённый ревью DC/source-workstream 15 проходящими cross-checks. H3-R2.2.1 проверил 14 сценариев startup/shutdown/reset/recovery, а H3-R2.2.2 — 7 316 переходов USB/pack/DPM/brownout/source-loss без небезопасного допуска или автоматического перезапуска; текущая аппаратная точка — H3-R2.2.3. Точные firmware-контракты последовательностей и handover импортированы в `config/h3_r2_transition_contract.json` и `config/h3_r2_handover_contract.json`. H2-R2.1.1 провёл ревью трёх native-проектов, 23 sheets, шести владельцев доменов, 242 точных MPN-групп и 1 197 позиций, а H2-R2.1.2 — 237 board groups, пять non-PCBA groups и 1 662 логических контакта. Более ранние live route FSUSB42MUX/C11355, точный тракт service-VBUS detector/latch/release и граница TCA9803DGKR/C2687966 Pack/Safety также прошли ревью. Точной импортированной pin/config
...
> модуля удалены; 6 GPIO S3, 5 GPIO заднего RP и 11 контактов M1 остаются
> резервами. Внешние, прямые внутренние после переворота плат и сервисные виды
> сгенерированы. H2-R2.1.3 материализовал 1 187 устанавливаемых позиций
> и 4 327 физических pins в трёх native-проектах KiCad. Все 4 323 логических контакта сведены в
> 4 065 подключённых endpoints, 258 явных no-connects и 826 канонических nets; все три проекта
> проходят KiCad ERC с нулём ошибок и предупреждений. H2-R2.1.4 свёл шесть доменов,
> 173 строки контроллеров, 51 межпроектную и 236 межлистовых nets; sync gate
...
| Пересечение с железом | ▶️ H0-R2, H1-R2.37, H2-R2.1.5, H3-R2.1, H3-R2.2.1 и H3-R2.2.2 прошли ревью; H3-R2.2.3 с inrush/load-step/watchdog/retained-fault — текущая точка. H3-R2.2.2 проводит все 7 316 переходов USB/pack/DPM/brownout/source-loss без небезопасного допуска или автоматического перезапуска. H3-R2.1 сводит 619 нагрузок, 224 rail-профиля и 2 266 состояний в 15 проходящих cross-checks. H3-R2.2.1 проводит 14 сценариев и требует нового квалифицированного физического KILL→RUN без автоматического перезапуска. Максимальный ток pack — 3,516 А против границы 8 А; 9 В × 3 А и 15 В × 2 А запускают любой профиль, 14 слишком тяжёлых USB-only состояний на 5 В × 3 А отклоняются. Электрический угол 3V3_MAIN — 3,046 А против границы допуска 3,2 А. Три native-проекта KiCad содержат 1 187 устанавливаемых позиций, 4 327 физических pins и 826 канонических nets без замечаний ERC; сведены шесть доменов, 173 строки контроллеров, 51 межпроектная и 236 межлистовых nets. Импортированной exact pin/config authority остаётся H1-R2.31; Pack/Safety идёт Hub GPIO42/43 → TCA9803DGKR → AON MSPM0 и не зависит от hard-kill. Карты двух RP, десять SMA 5+5, локальный S3 i8080-8 24 МГц, 10 настоящих NC M1, отдельный S3 reset на M1-36 и принятая модель 226 тел не изменились |
