...
> Физическое железо находится на H1-R2.33 и готово к визуальному принятию компоновки; точной импортированной pin/config
> authority остаётся прошедший ревью артефакт H1-R2.31, а актуальное locality-first размещение двух плат,
...
> Taoglas проходят текущие gates. Официальный manual Sinopine SP331RX контролирует
> номинальный XY 28,7×23,1 мм, шаг контактов 2,54 мм и краевой отступ 1,4 мм
> совпадающего семейства; аппаратный дизайн принимает двойную взаимоисключающую
> post-PCBA-посадку в резерве 30×24×8 мм и сохраняет 1,05 мм при требовании
> 0,70 мм после переноса C5 DBG10. Корпус вертикального
...
> и не нашла прямой замены. Обычный PCBA BOM не содержит приёмника: после reflow
> устанавливается ровно один модуль.
> Полные текущие внешние, прямые внутренние после переворота плат,
> сервисные виды и проверка разъёма сгенерированы; стабильная внешняя шелкография
> указывает роли UI и RF/power PCB, `R2-EVT1` и `REV A`, а изменяемый `H1-R2.xx`
> остаётся только в документации. При этом восстановлен
> пропущенный независимый набор Hub USB/RESET/BOOT/DBG10. Основной K331 использует
> толерантную 14-pad посадку; `AWM666V RX` с точным чертежом — вложенный
> деградированный семиканальный fallback. Выбранная RF-ветвь замыкается у запуска
> MMCX, а неиспользуемая изолируется там же — без U.FL, кабеля или live stub.
> Квалификация фактического модуля и ручной пайки относится к H5/H7; последующий
...
> активных U219-компонентов, но остаются три явных geometry-gate H1: точные
> значения/MPN и courtyards вспомогательных пассивов, контролируемая или
> измеренная геометрия NFC-петли и swept volume
...
| Пересечение с железом | ▶️ H0-R2 проведено ревью, физический H1-R2.33 геометрически завершён и ожидает визуального принятия, а импортированной machine pin/config authority остаётся H1-R2.31; точная легальная fixed-mux карта двух RP даёт задний I2C0 на GP4/5, независимый Cap I2C1 на GP30/31 и M5-профиль PIO2 на GP7/8; стабильная внешняя шелкография указывает роль каждой платы, `R2-EVT1` и `REV A`, а `H1-R2.xx` остаётся только в документации; десять SMA разделены 5+5, задний FPV MMCX не имеет хвоста в межплатный просвет, прямой i8080-8 24 МГц к `ER-TFT035IPS-6` + `ER-TPC035-6` и camera RX остаются локальны S3, а шлейф экрана направлен к антенному торцу, поэтому F5/F6 разворачивают память ILI9488 и touch-координаты FT6236 на 180°; TVP5150 остаётся спереди, а точный M1 на 80 контактов несёт один CVBS, 14 настоящих NC-резервов и не принимает механическую нагрузку корпуса; все 18 компонентов U219, NFC-loop и swept volume штатной антенны проходят fail-closed геометрию; H5/H7 отвечают за квалификацию полученного модуля и пайки |
