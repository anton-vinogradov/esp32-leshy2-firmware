...
> accepted and reviewed; hardware H2 is reviewed at H2-R2.1.5. The complete [H3-R2 global result](https://github.com/anton-vinogradov/esp32-leshy2/blob/main/docs/h3-r2-acceptance.md) is reviewed: 20 current artifacts and 129 recorded source hashes cross-check with zero mismatch or open analytical finding, while 51 physical-only rows remain owned by H5/H6/H8. Exact firmware-facing sequence, handover, watchdog/fault-display, digital-interface, RF/coexistence, thermal/fault and [H3 acceptance](../config/h3_r2_acceptance.json) contracts are imported fail-closed. H4-R2.0.1 freezes 24 exact current join inputs and matches all cross-repository H3 hashes; the current hardware point is H4-R2.0.2 for hardware-visible firmware-contract reconciliation. H2-R2.1.1 reviewed three native projects, 23 sheets, six domain owners, 240 exact MPN groups and 1,195 product positions, and H2-R2.1.2 reviewed 234 board groups, six explicit non-PCBA groups and 1,658 logical contacts. The earlier live FSUSB42MUX/C11355 route, exact service-VBUS detector/latch/release path and TCA9803DGKR/C2687966 Pack/Safety boundary are also reviewed. Its exact imported pin/config authority remains the reviewed
...
| Hardware intersection | ▶️ H0-R2, H1-R2.37, H2-R2.1.5 and the complete [H3-R2 result](https://github.com/anton-vinogradov/esp32-leshy2/blob/main/docs/h3-r2-acceptance.md) are reviewed; H4-R2.0.1 freezes 24 joined inputs and H4-R2.0.2 contract reconciliation is current. H3-R2.4 proves exact 20-MHz direct i8080-8, positive logic/timing margins, deterministic USB ownership and 80/80 M1 parity; H3-R2.5 imports ten permanent RF paths, 13 quiet contracts and all four three-nRF24 role mixes; H3-R2.6 imports 56 thermal profiles, 30 single-fault cases and the extended-operation policy without an autonomy claim. H3-R2.7 imports zero analytical findings, 51 owned physical residuals and one explicit F5/F6 i8080 obligation. Three native KiCad projects contain 1,185 fitted positions, 4,323 physical pins and 823 canonical nets with zero ERC findings; six domains, 173 controller pins, 52 cross-project and 238 cross-sheet nets reconcile. Imported exact pin/config authority remains H1-R2.31; Pack/Safety is Hub GPIO42/43 → TCA9803DGKR → AON MSPM0 and is not a hard-kill dependency. M1 has nine true NCs, latched `FAULT_KILL` on M1-35 and independent S3 reset on M1-36 |
