...
> **▶️ Current boundary: F2-R2.2 — six target projects.** R1 F0–F4
...
| S3/C5/RF-RP/Hub-RP/Pack/Safety projects | ▶️ F2-R2.2: the [exact six-target build matrix is reviewed](../config/f2_r2_build_matrix.json); five R1 structures are historical while six independent R2 project trees are current |
...
▶️ **`F2-R2.2` — current.** The [reviewed F2-R2.1 matrix](../config/f2_r2_build_matrix.json)
defines 12 locked offline argv-only jobs, 60 artifact paths, 16 map paths and 16
size gates. This substep creates six independent minimal production-SDK project
trees, including distinct RF-RP and Hub-RP identities, without running an R2
target build. The marker and evidence update together in every commit.
...
| **F2. Target projects and build system** | ▶️ Current: F2-R2.2 | Create six independent production-SDK projects from the reviewed matrix: ESP-IDF S3/C5, Pico SDK RF/Hub RP2354B and TI MSPM0 SDK ×2 | 12 debug/release configurations reproduce; every target consumes only its generated R2 pins |
...
The current boundary is `F2-R2.2`. The six-target toolchain, command, artifact
and size-gate matrix is reviewed. Create the six independent minimal target
project trees without running an R2 build. Generated R2 BSP work remains
dependent on the accepted hardware source; retained R1 build/QEMU evidence is
historical.
