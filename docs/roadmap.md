...
> accepted and reviewed; hardware H2 is reviewed at H2-R2.1.5. H3-R2.0 and H3-R2.1 are reviewed, and the complete H3-R2.2 workstream passes 14 startup/reset/recovery scenarios, 7,316 USB/pack/DPM/brownout/source-loss transitions, five protected-rail starts, four load-step envelopes and ten watchdog/fault-display cases without unsafe admission or automatic restart. The [H3-R2.3 analog result](https://github.com/anton-vinogradov/esp32-leshy2/blob/main/docs/analog-electrical-verification.md) is reviewed; H3-R2.4 is current for digital levels, timing, loading and direct i8080-8. Exact firmware-facing sequence, handover and watchdog/fault-display contracts are imported in `config/h3_r2_transition_contract.json`, `config/h3_r2_handover_contract.json` and `config/h3_r2_inrush_watchdog_contract.json`. H2-R2.1.1 reviewed three native projects, 23 sheets, six domain owners, 239 exact MPN groups and 1,195 product positions, and H2-R2.1.2 reviewed 234 board groups, five explicit non-PCBA groups and 1,656 logical contacts. The earlier live FSUSB42MUX/C11355 route, exact service-VBUS detector/latch/release path and TCA9803DGKR/C2687966 Pack/Safety boundary are also reviewed. Its exact imported pin/config authority remains the reviewed
...
| Hardware intersection | ▶️ H0-R2, H1-R2.37, H2-R2.1.5, H3-R2.1, the complete [H3-R2.2 result](https://github.com/anton-vinogradov/esp32-leshy2/blob/main/docs/power-transition-result.md) and the [H3-R2.3 analog result](https://github.com/anton-vinogradov/esp32-leshy2/blob/main/docs/analog-electrical-verification.md) are reviewed; H3-R2.4 digital levels, timing, loading and direct i8080-8 are current. H3-R2.2 passes 14 startup/reset/recovery scenarios, 7,316 source-transition cases, five starts, four load-step envelopes and ten fault cases without unsafe admission or automatic restart. Three native KiCad projects contain 1,185 fitted positions, 4,323 physical pins and 823 canonical nets with zero ERC findings; six domains, 173 controller pins, 52 cross-project and 238 cross-sheet nets reconcile. Imported exact pin/config authority remains H1-R2.31; Pack/Safety is Hub GPIO42/43 → TCA9803DGKR → AON MSPM0 and is not a hard-kill dependency. M1 has nine true NCs, latched `FAULT_KILL` on M1-35 and independent S3 reset on M1-36 |
