...
and canonical configure, build, clean, verify and artifact commands on top of
the reviewed [five-image environment lock](toolchains.md).
...
- `F2.0` — target/toolchain matrix.
  ...
  - ✅ `F2.0.2` — immutable SDK revisions, 26 archive URL/SHA-256 records for
    ...
    review.
  - ▶️ **`F2.0.3` — current:** define the single local/CI matrix and canonical
    configure, build, clean, verify and artifact commands.
...
`F2.0.3` exits when every physical target has an unambiguous local and CI
command path, named artifacts and a fail-closed preflight. Closing any substep
requires changing the exact marker on both landing and roadmap pages in the
same commit before advancing work.
