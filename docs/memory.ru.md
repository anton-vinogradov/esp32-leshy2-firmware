...
Лимиты не дублируются в CI вручную: машинный файл
[`config/s3_image_limits.json`](../config/s3_image_limits.json) читает
[`tools/check_s3_image_size.py`](../tools/check_s3_image_size.py). Второй OTA
slot не уменьшается автоматически ради новой функции.
...
## Почему исполняемый образ ограничен

S3 не содержит firmware трёх nRF24, CC1101, SA518, U214 или IR: эти realtime
трактовые функции исполняются на RP и C5. В образ S3 входят приложение, UI,
display/storage/audio, BLE/Wi‑Fi и междоменные протоколы. Шрифты сверх
минимального fault viewer, записи, карты, словари, captures и сменные пакеты UI
являются версионированными данными microSD, а не кодом.

Поэтому 6 МиБ — ранний измерительный порог, а не обещание угадать размер до
появления кода. При его достижении CI сохраняет map/size report и требует
сначала удалить случайно влинкованные assets и дубли библиотек. Только если
измеренный production-код после этого не помещается, разметка flash возвращается
на отдельное архитектурное ревью; rollback slot молча не отбирается.
