...
| Pack, Safety | TI MSPM0 SDK `2.11.00.07` | TI Arm Clang `4.0.0.LTS` | точный MSPM0C1106 поддержан SDK; компонент имеет статус ACTIVE |

Для TI выбран не самый новый компилятор вообще, а самый новый из явно
проверенных вместе с MSPM0 SDK `2.11.00.07`. Это уменьшает риск скрытой
несовместимости без деградации устройства. Оба MSPM0-образа имеют отдельные
проекты, но используют одну проверенную SDK/toolchain family.
...
F2.0.1 не означает, что образы уже собирались. Текущая F2.0.2 должна
зафиксировать URL и SHA-256 каждого устанавливаемого архива, SDK revision,
Python dependencies и воспроизводимый environment manifest. Затем F2.0.3
определит единые local/CI-команды, а фактические debug/release builds относятся
к F2.4.
