...
Задний Cap Bus принимает ровно один подписанный профиль одновременно. После
reset, при неизвестной identity или неверной подписи защищённая ветвь выключена,
внешние I/O изолированы, контакт 8 удерживается низким, а контакт 10 остаётся
безопасным для U214 входом. Смена профиля всегда проходит через полное
выключение и новую проверку identity, а не переопределяет контакты на ходу.
...
`POWER_EN` на контакте 8. U219 делит одну SPI-шину: CC1101 использует mode 0,
ST25R3916 — официальный M5-контракт mode 1/10 МГц; перед сменой режима оба CS
обязательно высокие.

CC1101 на U219 жёстко ограничен приёмом. Каждая raw-транзакция проходит allowlist,
который отклоняет `SFSTXON`, `STX`, запись PATABLE/TX FIFO,
...
fault/latency/range-тестами. Host-тесты доказывают только автомат состояний,
