...
почти бесплатен. Фабрика изготавливает и собирает две укомплектованные PCBA.
Владелец выполняет проверенную непаяную финальную сборку, включая установку
точного серийного дисплея, и первое полное включение по проверенному сценарию.

Ремонтопригодность не является приоритетом и не оправдывает увеличение цены,
габаритов или сложности компоновки и производства. Дополнительное удобство
обслуживания допустимо только без этих издержек. Требования безопасности,
надёжности, необходимого доступа для сборки и проверки, программирования
и восстановления прошивки сохраняются.
...
[Исправление управления mux C5](https://github.com/anton-vinogradov/esp32-leshy2/blob/main/docs/h6-r2-c5-mux-control-review.ru.md)
теперь имеет точную проверку исходной топологии: TS3USB221ERSER, два корпуса
SN74LV20APWR и три корпуса reset-sink NX3008NBKS сохраняют независимый
физический KILL. SEL следует явному запросу; разрешение соединения и удержание
Hub разделены. Это не реализует программный менеджер переключения и не
квалифицирует reset-to-high-Z, переходы питания, задержки переключения или
восстановление. Все четыре флага квалификации/реализации C5 в H3 остаются false.
Маршрут LV20A — явный предзаказ с MOQ 21 и оценочной ценой, не складской остаток.
...
фиксирует исправление площадок и сетей ROM-UART S3: сервисные TX/RX теперь
используют GPIO43/44, а Hub D2/D3 — GPIO7/8. Сгенерированные дескрипторы BSP
S3/Hub меняются; прежние build receipts не квалифицируют новую карту или сквозное
восстановление. Пустые Pack/Safety восстанавливаются прежде всего по SWD;
их текущие boot-manager entrypoints остаются заглушками.
...
содержит текущий закупочный снимок и явные условия допуска заказа, включая
`WBC16-1TLC`; закупка требует новой проверки непосредственно перед
...
[Отчёт об исправлении размещения](https://github.com/anton-vinogradov/esp32-leshy2/blob/main/docs/h6-r2-placement-repair.ru.md)
описывает центрированный держатель, энкодер слева, внутреннюю аудиогруппу и
утопленные порты. Эти механические изменения не меняют выбранные MPN,
назначения выводов/сетей или границу прошивки из шести доменов и не открывают
допуск заказа первого экземпляра.
...
согласованные внешние и прямые внутренние стороны после переворота плат и необходимый доступ для сборки, программирования и проверки.
...
native-проектов, 22 sheets, шести владельцев доменов, 252 точных component-groups
и 1 220 позиций. `H2-R2.1.2` проверил exact identities для 246 board groups,
шести non-PCBA groups и 1 615 контактов уникальных определений. Текущий
`H2-R2.1.3` содержит 1 210 устанавливаемых позиций и 4 321 физический вывод:
4 085 подключены, 236 явно не подключены, 789 глобальных канонических сетей.
Это счётчики исходной схемы, а не доказательство связности разведённой меди,
наличия источников у всех шин или отсутствия конфликтующих выходов;
электрическая и физическая проверки остаются обязательными до выпуска.
Одобренное пользователем обновление 9 сентября унифицирует все четыре USB-C
разъёма как `GCT USB4105-GF-A`. Удаляется одно больше не используемое определение
детали JAE и его 17 контактов, а не установленные endpoints: остаются 1 208 позиций
и 4 305 физических выводов. GPIO, транспортные API и все 13 C/H-файлов BSP побайтно
не изменились. Прежний результат 12 сборок сохранён без правок; проверка
воспроизводимости `F2-R2.5` остаётся впереди.
Затем исправление C5 от 14 сентября добавляет один корпус NAND и один bypass-
конденсатор без перенумерации прежних экземпляров. В C5/Hub BSP меняются только четыре
описания endpoints с FSUSB42 на TS3USB221E; остальные 11 C/H-файлов и все поля
GPIO, направлений и транспортных API не меняются относительно предыдущего входа.
PASS исходной топологии не означает реализацию менеджера или квалификацию переключения.
...
исправления RF-footprints и меток NC5 сохранены. Текущий H3 содержит условную
модель установленных TPS566231P и R67 = 1,65 кОм, но привязанная к native область
питания остаётся `review_required`. Фактические VIN, пульсации, падение напряжения
за защитой, запас PGTH, применимость RILIM для AON eFuse и запуск ещё требуют подтверждения.
Детали питания не менялись; синхронизация источников не квалифицирует ячейку
питания и не доказывает запуск под питанием. На
