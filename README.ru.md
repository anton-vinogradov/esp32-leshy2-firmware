...
железо завершило `H1-R2.37`, принято и прошло ревью; H2 прошло ревью как
`H2-R2.1.5`, весь DC/source-workstream H3-R2.1 проведён ревью, текущая аппаратная точка — `H3-R2.2.1`; H3-R2.1.2 провёл
ревью явной привязки 612 устанавливаемых питаемых экземпляров — 596 прямых и 16 косвенных — и шести внешних нагрузок, а H3-R2.1.3 — 224 проходящих профилей четырёх шин с минимальным запасом тока 30,560% и температуры кристалла 24,706 °C.
H3-R2.1.4 проводит ревью всех 75 source/pack-строк и безопасно допускает все
2 266 состояний; максимальный ток pack — 3,516 А, заряд всегда уступает системной нагрузке. H3-R2.1.5 сводит все 618 нагрузок, 224 rail-профиля и 2 266 состояний в 15 проходящих проверках. Точная фиксация входов
...
точную карту 80 контактов, 11 настоящих NC-резервов и отдельную
...
native-проектов, 23 sheets, шести владельцев доменов, 242 точных MPN-групп
и 1 197 позиций. `H2-R2.1.2` закрыл exact identities для 237 board groups,
пяти non-PCBA groups и 1 662 логических контактов. `H2-R2.1.3`
материализовал 1 187 устанавливаемых позиций и 4 327 физических pins
в трёх native-проектах KiCad. Все 4 323 логических контакта сведены в
4 063 подключённых endpoints, 260 явных no-connects и 827 канонических nets;
все три проекта проходят KiCad ERC с нулём ошибок и предупреждений. На
`H2-R2.1.4` сведены шесть доменов, 173 строки контроллеров, 50 межпроектных и
233 межлистовых nets; firmware sync gate результата `H2-R2.1.5` открыт. Теперь
H3 фиксирует эти входы; placement/routing
ещё не начинались, а
