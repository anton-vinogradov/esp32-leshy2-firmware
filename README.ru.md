...
контролирует все 249 закупаемых групп / 1 216 изделий без неназначенных маршрутов и с одним явным order-time sourcing gate `WBC16-1TLC`. H6.0.1 завершил placement, механический стек и пять сервисных коридоров microcoax; текущая аппаратная точка — `H6.0.3-R1`: обе платы имеют размер 80 x 150 мм, все 1 208 позиций остаются зафиксированы без жёстких конфликтов, а [актуальный срез разводки](https://github.com/anton-vinogradov/esp32-leshy2/blob/main/docs/h6-r2-current-routing.ru.md) связывает 5 351 чистых по DRC элементов меди с hash плат — 718 физических соединений замкнуто, 2 547 осталось в новой геометрии с 5-мм коридором внутренних сторон; UI analogue/audio/sense завершён; после 110 новых чистых соединений на RF/power в этом классе осталось 17. H3-R2.1.2
...
4 064 подключённых endpoints, 238 явных no-connects и 789 глобальных канонических / 823 локальных для плат nets;
оба проекта проходят KiCad ERC с нулём ошибок и предупреждений. На
`H2-R2.1.4` сведены шесть доменов, 173 строки контроллеров, 34 межпроектные и
228 межлистовых nets; firmware sync gate результата `H2-R2.1.5` открыт.
