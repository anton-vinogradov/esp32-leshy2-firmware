...
находится на `H1-R2.5`: корпуса Hub/Airband/резерв K331, точный MMCX и LDO получили
инкрементальное
...
`FPV RX 5.8G`. Контролируемый корпус и фабричный маршрут K331 ещё открыты. Точная ячейка
...
gate H3. Полный мокап R2, BSP, KiCad layout и разрешение заказа остаются открыты.
