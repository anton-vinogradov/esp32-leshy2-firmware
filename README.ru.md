...
контролирует все 249 закупаемых групп / 1 216 изделий без неназначенных маршрутов и с одним явным order-time sourcing gate `WBC16-1TLC`. H6.0.1 завершил placement, механический стек и пять сервисных коридоров microcoax; текущая аппаратная точка — `H6.0.3-R1`: обе платы имеют размер 80 x 150 мм, все 1 208 позиций остаются зафиксированы без жёстких конфликтов, а [актуальный срез разводки](https://github.com/anton-vinogradov/esp32-leshy2/blob/main/docs/h6-r2-current-routing.ru.md) связывает 5 432 чистых по DRC элемента меди с hash плат — 723 физических соединения замкнуто, 2 542 осталось в новой геометрии с 5-мм коридором внутренних сторон; UI analogue/audio/sense завершён; после 115 новых чистых соединений на RF/power в этом классе осталось 12. H3-R2.1.2
...
в двух native-проектах KiCad. Все 4 302 ledger-endpoints сведены в
4 064 подключённых endpoints, 238 явных no-connects и 788 глобальных канонических / 822 локальных для плат nets;
...
такая аппаратная проверка остаётся обязательной до производственного выпуска. На
...
H3 фиксирует эти входы. Размещение H6 завершено, разводка повторно
квалифицируется на платах 80 × 150 мм; routed release candidate ещё не готов.
