...
- Две заменяемые 18650 показываются отдельно и как контролируемая пара.
  Несовпадение, извлечение ячейки, ошибка контакта или неполная battery identity
  блокируют работу/зарядку и не могут быть обойдены программно.
