...
- Главный slow-I/O `TCA6424ARGJR` зафиксирован на I²C-адресе `0x22`, а pack
  admission — на `0x2A`. Обработчик общего IRQ определяет каждый активный
...
- Приёмное аудио сохраняет аппаратный bypass и работает, пока codec выключен
  или восстанавливается. Запись независимо выбирает активный receiver либо
  локальный микрофон; анализ микрофона и host-side VOX никогда не означают PTT.
- Codec playback включается только после физической готовности питания,
  I²C-readback на `0x19` и устойчивого I²S. Подключение наушников немедленно
  выключает reset-off speaker path; любой audio/bus/DMA/brownout fault сначала
  снимает codec playback и TX injection, затем выключает тракт.
...
- При входе в nRF-группу runtime питает все три развязанных endpoint, ждёт не
  менее `100 ms` и проверяет каждый радиомодуль. При выходе он паркует оба
  направления всех интерфейсов и подтверждает тишину трёх независимых
  детекторов прямой мощности до разряда общей шины.
- Верхний уровень одновременно активирует только одну квалифицированную группу
  ...
  Межгрупповая инъекция в Лаборатории может проверять перегрузку и восстановление,
  но никогда не создаёт runtime-разрешение.
...
- Передача native S3 2,4 ГГц и C5 2,4/5 ГГц использует разные внешние тракты и
  разные направленные actual-TX observations. Каждый lease связан с точными
  диапазоном/каналом, региональным профилем, калиброванной потерей тракта и
  идентичностью антенны; отсутствующее или противоречивое evidence выключает
  этот TX-профиль. Сильный входящий RF может консервативно задержать переход в
  quiet state, но не может разрешить передачу.
- CC1101 даёт раздельные аппаратные endpoints 315 МГц, 433 МГц и общий
  868/915 МГц. Два `BGS13SN8E6327XTSA1` изолируют каждую невыбранную ветвь с
  обоих концов; код `00` означает безопасную изоляцию. Диапазон меняется только
  при снятом питании CC, после чего до RX или TX повторно читаются identity и
...
- IR-learning одновременно получает устойчивую active-low огибающую 38 кГц от
  `TSOP95238TT` и измеренные циклы несущей 30–60 кГц от `TSMP95000TT`, сохраняя
  их provenance раздельно. Ограниченный по току излучатель `VSMY14940` тёмный
  при reset и hard STOP; экранированные `VEMD1060X01` и `TLV9061IDBVR`
  наблюдают реальный свет. Отсутствие света отзывает TX, а внешняя засветка
  может только задержать quiet и никогда не даёт разрешение.
- Broadcast receiver допускается через собственный power/interface gate и
  ...
  всё равно требует отдельного direct audio arm и независимо разрешённого
  AON-gated PTT. Режим мощности voice module управляется только low-or-release,
  но никогда активным высоким уровнем.
- `Si4732-A10-GSR` сохраняет четыре RX-режима на двух неизменных портах: FM/SW
  работает через защищённый FMI, а AM/LW — через отдельный защищённый
  не-50-Ом порт короткого loop-пода. Результаты и записи всегда содержат mode,
  физический порт и qualification антенны/пода; участок SW 2,3–25 МГц и
  неизвестные поды остаются явно неqualified экспериментами Лаборатории.
  В тракте нет RF switch или передатчика, а произвольный длинный коаксиал не
  является квалифицированным AM/LW-аксессуаром.
...
- M5 Unit и U214 Cap обслуживаются как профилированные и электрически независимые
  аксессуары. Presence-pin нет ни у одного разъёма: оба остаются без питания и
  в high-Z, пока не выбран exact signed manifest. Runtime включает только нужную
  ветвь, ждёт READY защищённой шины, затем разрешает изоляцию сигналов и
  проверяет profile-specific identity/readback. Несовпадение, внешнее питание,
  backfeed, overcurrent или stuck-bus защёлкивает эту ветвь выключенной и
  требует нового явного сеанса. Если внешний raw-SDR/RF-analysis модуль требует
  high-throughput transport, этот transport определяется его конкретным
  профилем и не подменяется низкоскоростным command link.
...
- Контролируемая батарея 2S показывает две заменяемые exact
  ...
  processor, и имеет независимые пути прошивки и восстановления. Его
  транслированный power-fail input является active-low и безопасен по уровням;
  standard GPIO притягивает общий системный interrupt только через
  passive-drain транзистор, поэтому reset или потеря питания admission не могут
  активно поднять общую линию. Глубоко
...
- VBUS сервисных портов C5/RP не питает устройство, а D+/D− отключаются при
  выключенной плате. Service attach — только диагностика: он не вооружает TX и
  не разрешает работу Лаборатории/Контролируемой зоны. Неверная идентичность
  fixture оставляет все линии high-Z; после recovery всегда начинается новая
  TX-off сессия.
