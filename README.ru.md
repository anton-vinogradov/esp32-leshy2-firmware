...
`H2-R2.1.5`, весь DC/source-workstream H3-R2.1 проведён ревью, а H3-R2.2.1
проверил 14 сценариев запуска, останова, сброса и восстановления. Текущая
аппаратная точка — `H3-R2.2.2`; точный state/timing-контракт для прошивки
[импортирован fail-closed](config/h3_r2_transition_contract.json). H3-R2.1.2
...
2 266 состояний; максимальный ток pack — 3,516 А, заряд всегда уступает системной нагрузке. H3-R2.1.5 сводит все 619 нагрузок, 224 rail-профиля и 2 266 состояний в 15 проходящих проверках. Точная фиксация входов
...
точную карту 80 контактов, 10 настоящих NC-резервов, отдельный S3 fault-UI reset
на контакте 36 и независимую
...
4 065 подключённых endpoints, 258 явных no-connects и 826 канонических nets;
все три проекта проходят KiCad ERC с нулём ошибок и предупреждений. На
`H2-R2.1.4` сведены шесть доменов, 173 строки контроллеров, 51 межпроектная и
236 межлистовых nets; firmware sync gate результата `H2-R2.1.5` открыт. Теперь
