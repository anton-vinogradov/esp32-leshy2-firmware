...
контролирует все 249 закупаемых групп / 1 216 изделий без неназначенных маршрутов и с одним явным order-time sourcing gate `WBC16-1TLC`. H6.0.1 завершил placement, механический стек и пять сервисных коридоров microcoax; текущая аппаратная точка — `H6.0.3-R1`: обе платы имеют размер 80 x 150 мм, все 1 208 позиций остаются зафиксированы без жёстких конфликтов, а [актуальный срез разводки](https://github.com/anton-vinogradov/esp32-leshy2/blob/main/docs/h6-r2-current-routing.ru.md) связывает 4 555 чистых по DRC элементов меди с hash плат — 600 физических соединений замкнуто, 2 665 осталось в новой геометрии с 5-мм коридором внутренних сторон. H3-R2.1.2
