...
HOST_RECEIVER_TEST := $(HOST_BUILD)/test_receiver_core
TARGET ?= all
CONFIG ?= debug
TARGET_PYTHON ?= python3.12
LOCKED_PYTHON ?= .toolchains/python/idf6_py3.12_env/bin/python

.PHONY: test u219-cap-policy-review r2-h2-sync-review f0-r2-review f1-r2-plan-review f1-r2-update-review f1-r2-receiver-review f1-r2-fault-review f1-r2-review f2-r2-plan-review f2-r2-matrix-review f2-r2-project-review f2-r2-bsp-review f2-r2-dispatcher-review host-test host-sanitize matrix-check source-layout-check build-policy-check f2-1-review f2-2-review f2-3-review f2-4-preflight-review f2-4-build-review f2-5-reproducibility-review f3-0-capability-review f3-0-runtime-plan-review f3-0-acceptance-plan-review f3-1-runtime-review f3-2-runtime-review f3-3-boundary-review f3-4-closure-review f4-0-capability-review f4-0-adapter-review f4-0-acceptance-review f4-1-source-review f4-1-core-review f4-1-endpoint-review f4-1-qemu-review f3-s3-run f3-s3-evidence-check f3-s3-scenario-run f3-s3-scenario-check capture-target-build locked-target-configure locked-target-build locked-target-clean locked-target-verify bsp-input-check bsp-generate bsp-check bsp-target-check target-projects-check targets-list target-preflight target-configure target-build target-verify target-artifacts target-clean clean

test: h3-current-scope-review u219-cap-policy-review r2-h2-sync-review f0-r2-review f1-r2-review f2-r2-plan-review f2-r2-matrix-review f2-r2-project-review f2-r2-bsp-review f2-r2-dispatcher-review
	python3 -m unittest tests.test_h0_r2_contract tests.test_r2_h2_sync_gate tests.test_f1_r2_portable tests.test_f2_r2_rebaseline tests.test_f2_r2_bsp tests.test_f2_r2_dispatcher tests.test_f2_r2_evidence_integrity tests.test_f2_r2_reproducibility tests.test_s3_psram_configuration tests.test_c5_psram_configuration tests.test_s3_psram_probe_receipt tests.test_c5_build_probe_receipt tests.test_psram_source_exports tests.test_h4_r2_correction_closure tests.test_h4_r2_acceptance tests.test_product_site tests.test_host_core

.PHONY: h3-current-scope-review
h3-current-scope-review:
	python3 -m unittest discover -s tests -p 'test_h3*.py'
...
f2-r2-project-review:
	python3 tools/check_f2_r2_target_projects.py

f2-r2-bsp-review:
	python3 tools/check_f2_r2_bsp.py

f2-r2-dispatcher-review:
	python3 tools/build_f2_r2_targets.py verify-matrix
	python3 tools/build_f2_r2_targets.py qualify --dry-run --summary-only
...
host-test: $(HOST_SAFETY_TEST) $(HOST_L2IP_TEST) $(HOST_HS_TEST) $(HOST_UPDATE_TEST) $(HOST_SYSTEM_TEST) $(HOST_RECEIVER_TEST)
	./$(HOST_SAFETY_TEST)
	./$(HOST_L2IP_TEST)
	./$(HOST_HS_TEST)
	./$(HOST_UPDATE_TEST)
...
$(HOST_RECEIVER_TEST): common/src/receiver_core.c host/tests/test_receiver_core.c common/include/leshy2/receiver_core.h
	mkdir -p $(HOST_BUILD)
	$(CC) $(CPPFLAGS) $(CFLAGS) common/src/receiver_core.c host/tests/test_receiver_core.c -o $@

clean:
	rm -f $(HOST_SAFETY_TEST) $(HOST_L2IP_TEST) $(HOST_HS_TEST) $(HOST_UPDATE_TEST) $(HOST_SYSTEM_TEST) $(HOST_RECEIVER_TEST)
